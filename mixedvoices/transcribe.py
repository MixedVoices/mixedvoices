...
def transcribe_with_whisper(audio_path):
    # TODO: Remove long silences from script before sending to whisper to speed up transcription and reduce cost
...
def create_combined_transcript(user_words: List[TranscriptionWord], assistant_words: List[TranscriptionWord]):
    last_speaker = None
    ...
    while user_index < len(user_words) or assistant_index < len(assistant_words):
        ...
        assistant_word = assistant_words[assistant_index] if assistant_index < len(assistant_words) else None
...
def transcribe_and_combine(user_audio_path, assistant_audio_path):
    user_transcript, user_words = transcribe_with_whisper(user_audio_path)
    assistant_transcript, assistant_words = transcribe_with_whisper(assistant_audio_path)
