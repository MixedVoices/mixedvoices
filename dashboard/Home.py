...
def apply_nav_styles():
    ...
    """ % (
        "0.4" if not (has_project and has_version) else "1",  # opacity
        "not-allowed" if not (has_project and has_version) else "pointer",  # cursor
        "none" if not (has_project and has_version) else "auto",  # pointer-events
        "0.4" if not (has_project and has_version) else "1",  # hover opacity
        "Select project and version first" if not has_project else "Select version first" if not has_version else "",  # tooltip text
        "block" if not (has_project and has_version) else "none"  # tooltip display
    )
